...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  else if (!strcmp(variable, "servo")) // 3250, 4875, 6500
  {
    ...
    if (val == 1) {
      ...
      actstate = fwd;
      ledcWrite(4, speed); // pin 12  右侧电机
      ledcWrite(3, 0);     // pin 13
      ledcWrite(5, speed * 0.75); // pin 14  调速左电机减慢
      ...
      ledcWrite(3, 0);
      ledcWrite(5, 0);
      ledcWrite(4, speed * 0.31);//0.32
      ledcWrite(6, speed * 0.43);//0.45


      //if      (actstate == fwd) { ledcWrite(4,speed*0.7); ledcWrite(6,    0); }
      // else if (actstate == rev) { ledcWrite(4,    0); ledcWrite(6,speed*0.6); }
      // else                      { ledcWrite(4,speed*0.7); ledcWrite(6,speed*0.6); }
      delay(600);
    }
    else if (val == 3) {
      Serial.println("Stop");
      ...
      ledcWrite(4, 0);
      ledcWrite(6, 0);
      ledcWrite(3, speed * 0.3);
      ledcWrite(5, speed * 0.29); //0.32

      //if      (actstate == fwd) { ledcWrite(3,    0); ledcWrite(5,speed*0.7); }
      //else if (actstate == rev) { ledcWrite(3,speed*0.6); ledcWrite(5,    0); }
      //else                      { ledcWrite(3,speed*0.6); ledcWrite(5,speed*0.7); }
      delay(600);
    }
    else if (val == 5) {
